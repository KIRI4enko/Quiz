...
from quiz.image_payload import question_image_urls_for_scope
...
class GameConsumer(AsyncWebsocketConsumer):
    ...
    async def new_question(self, event):
        ...
        if not image_urls and event.get("image_url"):
            ...
            text_data=json.dumps(
                {
                    ...
                    "image_url": image_urls[0] if image_urls else event.get("image_url"),
    ...
    def get_current_state(self):
        ...
        session = (
            ...
            .prefetch_related("current_game_question__question__gallery_images")
        ...
        if cq and cq.start_time:
            ...
            choices = [
                {"id": c.id, "text": c.text}
                for c in Choice.objects.filter(question=cq.question).order_by("id")
            ]
            img_urls = question_image_urls_for_scope(self.scope, cq.question)
            question_data = {
    ...
    def ws_moderator_start_question(self, session_id, user):
        ...
        next_game_question = (
            session.game_questions.filter(start_time__isnull=True)
            .select_related("question")
            .prefetch_related("question__gallery_images")
            .order_by("order")
            .first()
        )
        ...
        img_urls = question_image_urls_for_scope(self.scope, question)
        payload = {
            "game_question_id": next_game_question.id,
            "question_text": question.text,
            "image_urls": img_urls,
            "image_url": img_urls[0] if img_urls else None,
            "choices": [{"id": c.id, "text": c.text} for c in question.choices.all().order_by("id")],
